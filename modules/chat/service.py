...
from storage.models.message import Message as MessageModel
from storage.models.history import History as HistoryModel
...
class ChatService:
    ...
    async def chat(self, msg: str, current_history_id: int, user_id: int):
        ...
        summary_service = SummaryService(db=self.db, llm=self.llm, token_limit=self.token_limit)
    ...
    async def get_chat_history_by_history_id(self, history_id: int):
        res = await self.db.execute(
            select(MessageModel).where(MessageModel.history_id == history_id).order_by(MessageModel.timestamp, MessageModel.id)
        )
