...
def _get_rag_name(role_settings):
    """role_settings 可能是 RoleSetting 对象,也可能是 to_json() 后的 dict。"""
    if not role_settings:
        return None
    if isinstance(role_settings, dict):
        return role_settings.get("rag_name")
    return getattr(role_settings, "rag_name", None)
...
def rag_context_middleware(state: AgentState, runtime):
    ...
    role_rag_name = _get_rag_name(role_settings)
